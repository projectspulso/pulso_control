...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_status_geral'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_status_ideia'
) then create type pulso_status_ideia as enum (
    'RASCUNHO',
    'EM_DESENVOLVIMENTO',
    'APROVADA',
    'DESCARTADA'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_status_roteiro'
) then create type pulso_status_roteiro as enum (
    'RASCUNHO',
    'EM_REVISAO',
    'APROVADO',
    'PUBLICADO',
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_tipo_asset'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_status_conteudo'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_plataforma_tipo'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_status_post'
) then create type pulso_status_post as enum (
    'AGENDADO',
    'PUBLICADO',
    'ERRO_PUBLICACAO',
    'CANCELADO'
...
do $$ begin if not exists (
    select 1
    from pg_type
    where typname = 'pulso_tipo_evento_analytics'
...
create table if not exists pulso_core.canais_plataformas (
    ...
    -- ex: channel_id, @username, page_id
    nome_exibicao varchar(255),
    url_canal text,
    ativo boolean not null default true,
    configuracoes jsonb default '{}'::jsonb,
    -- tokens, escopos, etc. (guardar só o necessário)
    created_at timestamp without time zone default timezone('utc', now()),
    updated_at timestamp without time zone default timezone('utc', now()),
...
create table if not exists pulso_core.usuarios_internos (
    ...
    email varchar(200),
    papel varchar(100),
    -- ex.: 'ROTEIRISTA', 'EDITOR', 'GESTOR', etc.
    ativo boolean not null default true,
    created_at timestamp without time zone default timezone('utc', now()),
    updated_at timestamp without time zone default timezone('utc', now())
);
...
create table if not exists pulso_content.ideias (
    id uuid primary key default gen_random_uuid(),
    canal_id uuid references pulso_core.canais (id) on delete
    set null,
        serie_id uuid references pulso_core.series (id) on delete
    set null,
        titulo varchar(255) not null,
        descricao text,
        origem varchar(100),
        -- ex.: 'MANUAL', 'IA', 'TREND', etc.
        prioridade integer default 3,
        -- 1 = alta, 5 = baixa
        status pulso_status_ideia not null default 'RASCUNHO',
        tags text [],
        -- tags livres (além de pulso_core.tags)
        linguagem varchar(10) default 'pt-BR',
        criado_por uuid references pulso_core.usuarios_internos (id) on delete
    set null,
        metadata jsonb default '{}'::jsonb,
        created_at timestamp without time zone default timezone('utc', now()),
        updated_at timestamp without time zone default timezone('utc', now())
...
create table if not exists pulso_content.roteiros (
    id uuid primary key default gen_random_uuid(),
    ideia_id uuid references pulso_content.ideias (id) on delete
    set null,
        titulo varchar(255) not null,
        ...
        criado_por uuid references pulso_core.usuarios_internos (id) on delete
    set null,
        revisado_por uuid references pulso_core.usuarios_internos (id) on delete
    set null,
        metadata jsonb default '{}'::jsonb,
        created_at timestamp without time zone default timezone('utc', now()),
        updated_at timestamp without time zone default timezone('utc', now()),
...
create table if not exists pulso_content.conteudo_variantes (
    ...
    -- ex.: "Versão A", "Hook forte", "Corte 1"
    plataforma_tipo pulso_plataforma_tipo,
    -- pode ser null se ainda não definido
    status pulso_status_conteudo not null default 'RASCUNHO',
    titulo_publico varchar(255),
    descricao_publica text,
    legenda text,
    -- texto final que vai na legenda da plataforma
    hashtags text [],
    linguagem varchar(10) default 'pt-BR',
...
create table if not exists pulso_assets.assets (
    ...
    -- ex: path no Supabase Storage ou URL externa
    provedor varchar(100) default 'SUPABASE',
    -- SUPABASE, DRIVE, EXTERNO, etc.
...
create table if not exists pulso_assets.conteudo_variantes_assets (
    ...
    -- ex.: 'VIDEO_PRINCIPAL', 'AUDIO_TTS', 'THUMBNAIL', 'LEGENDAS'
...
create table if not exists pulso_distribution.posts (
    ...
    -- id do vídeo/post na plataforma
...
create table if not exists pulso_automation.workflow_execucoes (
    ...
    -- IDEIA, ROTEIRO, CONTEUDO, POST, etc.
    entidade_id uuid,
...
create table if not exists pulso_analytics.eventos (
    id uuid primary key default gen_random_uuid(),
    post_id uuid references pulso_distribution.posts (id) on delete
    set null,
        plataforma_id uuid references pulso_core.plataformas (id) on delete
    set null,
        tipo pulso_tipo_evento_analytics not null,
        quantidade integer not null default 1,
        valor_numerico numeric(18, 4),
        -- para watch time, CTR, etc. se necessário
        metadata jsonb default '{}'::jsonb,
