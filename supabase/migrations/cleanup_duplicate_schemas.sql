-- =====================================================
-- MIGRATION: Remover schemas duplicados (sem prefixo pulso_)
-- =====================================================
-- Este script remove os schemas que foram criados sem o prefixo "pulso_"
-- e mantém apenas os schemas corretos: pulso_core, pulso_content, pulso_distribution, pulso_analytics
-- 1. Remover views que apontam para os schemas errados
...
-- 2. Remover schemas duplicados (se existirem e estiverem vazios ou com dados duplicados)
-- IMPORTANTE: Só execute isso se você tiver certeza que os dados corretos estão nos schemas pulso_*
-- Remover schema 'core' (se existir)
DROP SCHEMA IF EXISTS core CASCADE;
-- Remover schema 'content' (se existir)
DROP SCHEMA IF EXISTS content CASCADE;
-- Remover schema 'distribution' (se existir)
DROP SCHEMA IF EXISTS distribution CASCADE;
-- Remover schema 'analytics' (se existir)
DROP SCHEMA IF EXISTS analytics CASCADE;
-- Remover schema 'assets' (se existir) - deve usar pulso_assets
DROP SCHEMA IF EXISTS assets CASCADE;
-- 3. Recriar views públicas apontando para os schemas CORRETOS (pulso_*)
...
GRANT SELECT,
    ...
    authenticated;
COMMENT ON VIEW public.ideias IS 'View pública de ideias (pulso_content.ideias)';
...
GRANT SELECT,
    ...
    authenticated;
COMMENT ON VIEW public.roteiros IS 'View pública de roteiros (pulso_content.roteiros)';
...
-- 4. Recriar triggers INSTEAD OF para permitir INSERT/UPDATE/DELETE nas views
...
'reload schema';
-- 6. Verificação final
SELECT 'Limpeza concluída! Schemas duplicados removidos.' as status;
SELECT schema_name
FROM information_schema.schemata
WHERE schema_name LIKE 'pulso_%'
    OR schema_name = 'assets'
