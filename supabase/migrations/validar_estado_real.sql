...
-- 6. RESUMO FINAL
SELECT 
  '6. RESUMO FINAL' as secao,
  (SELECT COUNT(*) FROM pulso_content.ideias WHERE status = 'APROVADA') as ideias_aprovadas,
  (SELECT COUNT(*) FROM pulso_content.ideias WHERE status = 'RASCUNHO') as ideias_rascunho,
  (SELECT COUNT(*) FROM pulso_content.roteiros WHERE status = 'RASCUNHO') as roteiros_rascunho,
  (SELECT COUNT(*) FROM pulso_content.roteiros WHERE status = 'APROVADO') as roteiros_aprovados_DEVE_SER_ZERO,
  (SELECT COUNT(*) FROM pulso_content.audios) as audios_DEVE_SER_ZERO,
  (SELECT COUNT(*) FROM pulso_content.pipeline_producao) as total_pipeline;
...
-- 7. VALIDAÇÃO CRÍTICA
SELECT 
  '7. VALIDACAO CRITICA' as titulo,
  CASE 
    WHEN (SELECT COUNT(*) FROM pulso_content.roteiros WHERE status IN ('APROVADO', 'EM_REVISAO')) = 0
      AND (SELECT COUNT(*) FROM pulso_content.audios) = 0
      AND (SELECT COUNT(*) FROM pulso_content.pipeline_producao WHERE status = 'PRONTO_PUBLICACAO') = 0
    THEN 'SUCESSO - Banco sem dados mock, apenas dados reais'
