...
-- =====================================================
CREATE OR REPLACE FUNCTION pulso_content.fn_calcular_proxima_data_publicacao()
...
DECLARE
    ...
    horarios int[] := ARRAY[9, 15, 21]; -- 9h, 15h, 21h
    hora_atual int;
    indice_hora int;
    dia_atual date;
BEGIN
    -- Buscar a última data de publicação agendada
    ...
    IF ultima_data IS NULL THEN
        RETURN '2025-12-10 09:00:00'::timestamp with time zone;
    END IF;
    
    -- Extrair hora da última data
    hora_atual := EXTRACT(HOUR FROM ultima_data)::int;
    dia_atual := ultima_data::date;
    
    -- Determinar o próximo horário disponível
    IF hora_atual < 9 THEN
        -- Antes das 9h, próximo slot é 9h do mesmo dia
        proxima_data := (dia_atual || ' 09:00:00')::timestamp with time zone;
    ELSIF hora_atual >= 9 AND hora_atual < 15 THEN
        -- Entre 9h e 15h, próximo slot é 15h do mesmo dia
        proxima_data := (dia_atual || ' 15:00:00')::timestamp with time zone;
    ELSIF hora_atual >= 15 AND hora_atual < 21 THEN
        -- Entre 15h e 21h, próximo slot é 21h do mesmo dia
        proxima_data := (dia_atual || ' 21:00:00')::timestamp with time zone;
    ELSE
        -- Depois das 21h, próximo slot é 9h do dia seguinte
        proxima_data := ((dia_atual + INTERVAL '1 day')::date || ' 09:00:00')::timestamp with time zone;
    END IF;
    
    -- Verificar se o slot calculado já está ocupado
    WHILE EXISTS (
        SELECT 1 FROM pulso_content.pipeline_producao 
        WHERE data_publicacao = proxima_data
    ) LOOP
        -- Se já existe, avançar para o próximo slot
        hora_atual := EXTRACT(HOUR FROM proxima_data)::int;
        dia_atual := proxima_data::date;
        
        IF hora_atual = 9 THEN
            proxima_data := (dia_atual || ' 15:00:00')::timestamp with time zone;
        ELSIF hora_atual = 15 THEN
            proxima_data := (dia_atual || ' 21:00:00')::timestamp with time zone;
        ELSE -- hora_atual = 21
            proxima_data := ((dia_atual + INTERVAL '1 day')::date || ' 09:00:00')::timestamp with time zone;
        END IF;
...
-- =====================================================
CREATE OR REPLACE FUNCTION pulso_content.trg_auto_agendar_publicacao()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    -- Se data_publicacao já foi definida manualmente, não sobrescrever
    IF NEW.data_publicacao IS NOT NULL THEN
        RETURN NEW;
    END IF;
...
-- COMENTÁRIOS
-- =====================================================
COMMENT ON FUNCTION pulso_content.fn_calcular_proxima_data_publicacao() IS 
'Calcula a próxima data/hora disponível para publicação baseado no calendário existente. 
Usa slots de 9h, 15h e 21h. Se um slot já estiver ocupado, avança para o próximo.';

COMMENT ON FUNCTION pulso_content.trg_auto_agendar_publicacao() IS 
'Trigger function que automaticamente atribui data_publicacao e data_prevista 
...
BEGIN
    -- Buscar primeira ideia e canal
    SELECT id INTO test_ideia_id FROM pulso_content.ideias LIMIT 1;
    SELECT id INTO test_canal_id FROM pulso_core.canais LIMIT 1;
...
-- =====================================================
SELECT 
    'Próxima data disponível:' as info,
    pulso_content.fn_calcular_proxima_data_publicacao() as proxima_data,
    TO_CHAR(pulso_content.fn_calcular_proxima_data_publicacao(), 'DD/MM/YYYY HH24:MI') as formatado;
