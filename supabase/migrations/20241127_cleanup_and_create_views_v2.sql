...
LEFT JOIN pulso_core.series s
  ON s.id = co.serie_id
-- FALLBACK: join com ideias para pegar titulo e status se variante não existir
LEFT JOIN pulso_content.ideias i 
  ON i.id = p.ideia_id;
...
COMMENT ON VIEW public.vw_pulso_calendario_publicacao_v2 IS 
'View V2 para calendário editorial - usa conteudo_variantes.titulo_publico com fallback para ideias.titulo';

COMMENT ON VIEW public.vw_pulso_pipeline_com_assets_v2 IS 
'View V2 completa com assets - inclui thumbnails, vídeos, áudios vinculados às variantes';
