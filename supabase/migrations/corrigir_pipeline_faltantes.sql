...
SELECT 
  ...
  (SELECT COUNT(*) FROM pulso_content.ideias) - (SELECT COUNT(*) FROM pulso_content.pipeline_producao) as faltando;

-- 2. IDENTIFICAR IDEIAS SEM PIPELINE
SELECT 
  'IDEIAS SEM PIPELINE' as categoria,
  i.id as ideia_id,
  i.titulo,
...
SELECT 
  ...
  jsonb_build_object(
    'canal_id', i.canal_id::text,
    'serie_id', i.serie_id::text,
    'origem', 'correcao_automatica',
    'corrigido_em', NOW()
  ) as metadata,
  NOW() as created_at,
  NOW() as updated_at
...
SELECT 
  'APOS INSERCAO' as momento,
  (SELECT COUNT(*) FROM pulso_content.ideias) as total_ideias,
  (SELECT COUNT(*) FROM pulso_content.pipeline_producao) as total_pipeline,
  (SELECT COUNT(*) FROM pulso_content.ideias) - (SELECT COUNT(*) FROM pulso_content.pipeline_producao) as diferenca;

-- 5. VERIFICAR DISTRIBUIÇÃO DE STATUS
SELECT 
  'DISTRIBUICAO STATUS' as categoria,
  status::text,
  COUNT(*) as total,
  CASE 
...
SELECT 
  ...
  CASE 
    WHEN (SELECT COUNT(*) FROM pulso_content.ideias) = (SELECT COUNT(*) FROM pulso_content.pipeline_producao)
    THEN 'OK - Pipeline completo (1 item por ideia)'
    ELSE 'ERRO - Ainda há diferença'
  END as resultado,
  (SELECT COUNT(*) FROM pulso_content.ideias) as ideias,
  (SELECT COUNT(*) FROM pulso_content.pipeline_producao) as pipeline;
