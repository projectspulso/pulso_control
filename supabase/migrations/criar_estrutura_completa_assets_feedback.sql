...
CREATE TABLE IF NOT EXISTS pulso_content.personagens (
    ...
    tipo TEXT NOT NULL CHECK (tipo IN ('VOZ', 'AVATAR', 'APRESENTADOR', 'NARRADOR')),
    
    -- Características
    genero TEXT CHECK (genero IN ('MASCULINO', 'FEMININO', 'NEUTRO')),
    idioma TEXT DEFAULT 'pt-BR',
    tom TEXT, -- calmo, energético, misterioso, etc.
    idade_aproximada TEXT, -- jovem, adulto, idoso
    
    -- Configurações técnicas
    voz_id TEXT, -- ID da voz no provedor (OpenAI, ElevenLabs, etc.)
    avatar_id TEXT, -- ID do avatar (D-ID, HeyGen, etc.)
    provedor TEXT, -- openai, elevenlabs, azure, etc.
    ...
    custo_por_uso NUMERIC(10,4), -- custo em créditos/reais
...
CREATE TABLE IF NOT EXISTS pulso_content.thumbnails (
    ...
    largura_px INTEGER,
    altura_px INTEGER,
    formato TEXT, -- jpg, png, webp
    tamanho_bytes BIGINT,
    
    -- Informações de geração
    tipo_geracao TEXT CHECK (tipo_geracao IN ('IA', 'MANUAL', 'TEMPLATE')),
    prompt_usado TEXT, -- se gerado por IA (DALL-E, Midjourney, etc.)
    modelo_ia TEXT, -- dall-e-3, midjourney-v6, etc.
    provedor TEXT, -- openai, midjourney, stable-diffusion, etc.
    
    -- Elementos visuais
    titulo_texto TEXT, -- texto que aparece na thumb
    estilo TEXT, -- dark, minimalista, colorido, dramático, etc.
    cores_principais TEXT[], -- ['#FF0000', '#00FF00']
    
    -- Versão e testes
    versao INTEGER DEFAULT 1,
    variante TEXT, -- A, B, C (para A/B testing)
    is_principal BOOLEAN DEFAULT false, -- thumb escolhida para publicação
...
CREATE INDEX IF NOT EXISTS idx_thumbnails_principal ON pulso_content.thumbnails(is_principal) WHERE is_principal = true;
...
CREATE TABLE IF NOT EXISTS pulso_content.feedbacks (
    ...
    entidade_tipo TEXT NOT NULL CHECK (entidade_tipo IN ('IDEIA', 'ROTEIRO', 'AUDIO', 'VIDEO', 'THUMBNAIL')),
    entidade_id UUID NOT NULL, -- ID da ideia, roteiro, etc.
    
    -- Quem avaliou
    avaliador_tipo TEXT CHECK (avaliador_tipo IN ('HUMANO', 'IA', 'METRICAS_REAIS')),
    avaliador_id TEXT, -- user_id ou nome do modelo de IA
    
    -- Avaliação
    nota NUMERIC(3,1) CHECK (nota BETWEEN 0 AND 10), -- 0.0 a 10.0
    aprovado BOOLEAN,
    ...
    -- Categorização
    tags TEXT[], -- ["muito_bom", "precisa_melhorar", "viral"]
    categoria_feedback TEXT, -- POSITIVO, NEGATIVO, NEUTRO, MISTO
    ...
    tempo_medio_visualizacao NUMERIC(5,2), -- segundos
    taxa_retencao NUMERIC(5,2), -- porcentagem
...
CREATE TABLE IF NOT EXISTS pulso_content.metricas_publicacao (
    ...
    url_publicacao TEXT,
    post_id TEXT, -- ID na plataforma
    
    -- Data de publicação
    data_publicacao TIMESTAMPTZ NOT NULL,
    hora_publicacao TEXT, -- formato HH:MM para análise de melhor horário
    dia_semana INTEGER, -- 0-6 para análise de melhor dia
    ...
    saves INTEGER DEFAULT 0, -- salvamentos (Instagram, TikTok)
    
    -- Métricas avançadas
    tempo_medio_visualizacao NUMERIC(5,2), -- segundos
    taxa_retencao NUMERIC(5,2), -- porcentagem
    taxa_cliques NUMERIC(5,2), -- CTR
    taxa_conversao NUMERIC(5,2), -- se houver CTA
    ...
    performance_vs_media TEXT CHECK (performance_vs_media IN ('MUITO_ACIMA', 'ACIMA', 'MEDIA', 'ABAIXO', 'MUITO_ABAIXO')),
    percentil INTEGER CHECK (percentil BETWEEN 0 AND 100), -- top 10%, top 50%, etc.
    
    -- Dados demográficos
    pais_principal TEXT,
    idade_principal TEXT, -- 18-24, 25-34, etc.
...
ALTER TABLE pulso_content.ideias ADD COLUMN IF NOT EXISTS personagem_sugerido_id UUID REFERENCES pulso_content.personagens(id);
ALTER TABLE pulso_content.ideias ADD COLUMN IF NOT EXISTS nota_ia_inicial NUMERIC(3,1);
ALTER TABLE pulso_content.ideias ADD COLUMN IF NOT EXISTS potencial_viral_ia NUMERIC(3,1);

-- Adicionar colunas em roteiros para personagem usado
ALTER TABLE pulso_content.roteiros ADD COLUMN IF NOT EXISTS personagem_id UUID REFERENCES pulso_content.personagens(id);
ALTER TABLE pulso_content.roteiros ADD COLUMN IF NOT EXISTS nota_qualidade_ia NUMERIC(3,1);

-- Adicionar colunas em audios (se não existirem)
DO $$ 
BEGIN
    -- Verificar se tabela audios existe no schema correto
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'pulso_content' AND table_name = 'audios') THEN
        ALTER TABLE pulso_content.audios ADD COLUMN IF NOT EXISTS personagem_id UUID REFERENCES pulso_content.personagens(id);
        ALTER TABLE pulso_content.audios ADD COLUMN IF NOT EXISTS qualidade_voz_ia NUMERIC(3,1);
    END IF;
END $$;
...
CREATE TRIGGER update_personagens_updated_at
    BEFORE UPDATE ON pulso_content.personagens
    FOR EACH ROW
...
LEFT JOIN pulso_content.feedbacks f ON f.entidade_id = r.id AND f.entidade_tipo = 'ROTEIRO'
WHERE p.ativo = true
GROUP BY p.id, p.nome, p.tipo, p.tom, p.total_usos
...
LEFT JOIN pulso_content.metricas_publicacao m ON m.ideia_id = t.ideia_id AND t.is_principal = true
