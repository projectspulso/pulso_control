...
FROM assets.audios;
RAISE NOTICE 'assets.audios tem % registros',
audio_count;
ELSE RAISE NOTICE 'Tabela assets.audios não existe';
END IF;
-- Contar videos
IF EXISTS (
    SELECT 1
...
FROM assets.videos;
RAISE NOTICE 'assets.videos tem % registros',
video_count;
ELSE RAISE NOTICE 'Tabela assets.videos não existe';
END IF;
END $$;
-- =========================================================================
...
ALTER TABLE pulso_content.pipeline_producao DROP CONSTRAINT IF EXISTS pipeline_producao_audio_id_fkey CASCADE;
...
DO $$ BEGIN IF EXISTS (
    ...
    WHERE table_schema = 'assets'
        AND table_name = 'audios'
) THEN -- Se pulso_content.audios não existir, criar
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'pulso_content'
        AND table_name = 'audios'
) THEN -- Criar tabela em pulso_content
...
DO $$ BEGIN IF EXISTS (
    ...
    WHERE table_schema = 'assets'
        AND table_name = 'videos'
) THEN -- Se pulso_content.videos não existir, criar
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'pulso_content'
...
ADD CONSTRAINT pipeline_producao_audio_id_fkey FOREIGN KEY (audio_id) REFERENCES pulso_content.audios(id) ON DELETE
SET NULL;
...
DO $$
DECLARE schema_exists BOOLEAN;
BEGIN -- Verificar se schema assets ainda existe
SELECT EXISTS (
        SELECT 1
        FROM information_schema.schemata
        WHERE schema_name = 'assets'
    ) INTO schema_exists;
IF schema_exists THEN RAISE WARNING 'Schema assets ainda existe! Algo deu errado.';
ELSE RAISE NOTICE '✅ Schema assets eliminado com sucesso!';
END IF;
...
IF EXISTS (
    ...
    WHERE table_schema = 'pulso_content'
        AND table_name = 'audios'
) THEN RAISE NOTICE '✅ pulso_content.audios existe';
ELSE RAISE WARNING '❌ pulso_content.audios NÃO existe';
END IF;
IF EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'pulso_content'
        AND table_name = 'videos'
) THEN RAISE NOTICE '✅ pulso_content.videos existe';
ELSE RAISE WARNING '❌ pulso_content.videos NÃO existe';
END IF;
...
) THEN RAISE NOTICE '✅ View public.pipeline_producao existe';
ELSE RAISE WARNING '❌ View public.pipeline_producao NÃO existe';
END IF;
END $$;
-- Mostrar resumo final
SELECT 'pulso_content' as schema,
    COUNT(*) FILTER (
    ...
    COUNT(*) FILTER (
        WHERE table_name = 'metricas_publicacao'
    ) as metricas
FROM information_schema.tables
WHERE table_schema = 'pulso_content'
    AND table_name IN (
