...
DELETE FROM pulso_content.personagens
WHERE nome = 'Pulso';
...
VALUES (
        ...
        jsonb_build_object(
            ...
            jsonb_build_object(
                'provedor',
                'openai',
                -- ou 'elevenlabs' quando tiver
                'voz_base_id',
                'alloy',
                -- ID da voz no provedor
                'modelo',
                'tts-1-hd'
            ),
            -- Variações por canal
            ...
            jsonb_build_object(
                ...
                jsonb_build_object(
                    'visual',
                    'pulso_psicologia.png',
                    'cores',
                    ARRAY ['#8B7355', '#D4A574', '#F5E6D3'],
                    -- Terra, acolhedor
                    'expressao',
                    'calmo, sábio, acolhedor',
                    'voz',
                    jsonb_build_object(
                        'speed',
                        0.9,
                        -- 10% mais devagar (OpenAI)
                        'pitch',
                        -0.1,
                        -- Tom mais grave (ElevenLabs/Google)
                        'stability',
                        0.8,
                        -- Mais estável (ElevenLabs)
                        'estilo',
                        'reflexivo'
                    ),
                    'descricao',
                    'Pulso assume uma forma calma e acolhedora, com cores terrosas. Voz pausada e reflexiva para conteúdos de psicologia.'
                ),
                -- FATOS INUSITADOS: Tom animado, rápido, curioso
                'fatos_inusitados',
                jsonb_build_object(
                    'visual',
                    'pulso_fatos_inusitados.png',
                    'cores',
                    ARRAY ['#FF6B35', '#F7931E', '#FDC830'],
                    -- Vibrante, energia
                    'expressao',
                    'curioso, empolgado, surpreso',
                    'voz',
                    jsonb_build_object(
                        'speed',
                        1.1,
                        ...
                        'stability',
                        0.5,
                        -- Mais variado/animado
                        'estilo',
                        'empolgado'
                    ),
                    'descricao',
                    'Pulso fica empolgado e curioso, com cores vibrantes. Voz rápida e animada para surpreender com fatos inusitados.'
                ),
                -- TECNOLOGIA: Tom moderno, profissional, preciso
                'tecnologia',
                jsonb_build_object(
                    'visual',
                    'pulso_tecnologia.png',
                    'cores',
                    ARRAY ['#667EEA', '#764BA2', '#00D4FF'],
                    -- Futurista, tech
                    'expressao',
                    'confiante, moderno, inovador',
                    'voz',
                    jsonb_build_object(
                        'speed',
                        1.0,
                    ...
                    'descricao',
                    'Pulso assume forma futurista com cores tech (azul/roxo). Voz moderna e precisa para conteúdos de tecnologia.'
                ),
                ...
                jsonb_build_object(
                    ...
                    'descricao',
                    'Forma padrão do Pulso para canais sem variação específica.'
                )
            ),
            ...
            jsonb_build_object(
                'tagline',
                'Seu guia adaptável pelo conhecimento',
                'filosofia',
                'Um personagem, infinitas formas. O Pulso se transforma para se conectar melhor com cada tema.',
                'identidade',
                'Mantém a essência mas adapta forma e tom ao contexto'
            )
        ),
        true -- ativo
    );
