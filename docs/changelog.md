...
- **CORRIGIDO no mesmo dia (dono escolheu a opção 1):** o critério "tem gancho" passou a ser a nota real do avaliador de gancho (`notaHook >= 3`) e os pontos de parágrafo foram para gancho e duração — gancho 30 · chamada 25 · duração 30 · tamanho 15. 80 agora exige gancho + chamada + duração no alvo. Os 4 roteiros ainda em rascunho foram renotados (nota antiga guardada em `quality_score_antigo`): #227 foi a 100, e #251/#252/#253 seguram por motivo real (gancho 2/5). Casos de teste da nota nova conferidos.
- **O achado, como estava:** a nota de qualidade conta PARÁGRAFOS no critério "tem gancho" (`paragrafos >= 2`). O gerador escreve em bloco único (média 1,0–1,2 parágrafo desde setembro), então quase todo roteiro fica em 65 com o gancho real em 4,4/5. Desde a semana de 07/09 nenhum roteiro chegou a 80 — a aprovação automática, ligada, na prática não aprova nada; 27 dos 28 roteiros na fila têm a nota derrubada por isso.
...
- **A fila já agendada foi re-datada para 1/dia (18h), na mesma ordem.** Sem isso, os 6 vídeos das 21h nunca sairiam: o publicador ignora agendamento vencido há mais de 12 horas, e com 1 vaga por dia o das 21h sempre vencia na fila. 12 vídeos, de 23/09 a 04/10.
- **Teste de tema novo com trava** (`lib/agenda/teste-temas.ts`, regras em `configuracoes.teste_temas`, números do dono): 1 tema em teste por vez; 7 dias entre tentativas; corte na hora se uma tentativa fizer menos de 40% do piso; aprovado depois de 3 tentativas. O piso é a mediana do pior tema normal **na mesma rede e na mesma idade** (2 dias) — comparar vídeo de 2 dias com mediana de vídeos de 40 dias, ou Facebook com TikTok, reprovaria tudo. Tema em teste = canal com `metadata.teste.status = 'em_teste'`; os 14 canais que já rodavam não mudam em nada.
- **O tema em teste não disputa a vaga pelo score** — perderia sempre (sem histórico) ou ganharia sempre ("parado desde sempre" pesa no score). Entra por convite no roteador, só com vídeo pronto, e fica fora pelo intervalo. O publicador tem a mesma checagem como última porta, para data posta à mão; falha fechada se não conseguir ler os canais.
- **O veredito é gravado** em `canais.metadata.teste` pela rota `agenda/popular` antes de planejar: reprovado não volta sozinho se o vídeo crescer depois.

### Segurança (08/09)
- **RLS ligada nas 20 tabelas de `pulso_content`, com prova de comportamento.** Migrations `065` e `067`. Não foi "ligar a RLS": as tabelas tinham policies chamadas **"Permitir tudo para todos"** (`using (true)`) dormentes — ligar a RLS as acordaria e o banco ficaria tão aberto quanto antes, com a auditoria de catálogo aprovando. A saída foi **policy restritiva** (restritivas multiplicam com AND; permissivas somam com OR), que anula a antiga sem apagá-la. Ordem obrigatória: permissiva do interno → restritiva → `enable`.
- **Provado por simulação, não por catálogo:** um usuário logado **fora** de `usuarios_internos` vê `roteiros 0 · ideias 0 · videos 0 · metricas 0`; o dono vê `220 · 283 · 67`. A `067` nasceu dessa prova — a restritiva das 3 tabelas do hub ainda tinha um ramo "ou publicado" que deixava o intruso ler 212 ideias com a `checagem` junto. Ler catálogo não mostrava isso.
- **A chave pública não lê mais NENHUMA tabela de `pulso_content`.** Só as duas views mínimas do hub. Sequência: `062` views → hub corrigido → hub publicado (`2b69387`) → **confirmado nos `edge_logs` do Supabase que o hub vivo passou a chamar `vw_hub_videos`/`vw_hub_links`** → `064` e `066` revogam. Conferido depois: `ideias`/`pipeline_producao`/`metricas_publicacao` negados, hub 200 com 120 cards, ficha 200, app 200.
- **A chave pública lia 220 roteiros inteiros.** Medido com a `anon` — a que viaja no bundle do navegador de qualquer visitante do hub: `roteiros` 220 linhas com o `conteudo_md` completo, `videos` 67, `plano_publicacao` 20, `conteudos` 5, `personagens` 1. **Fechado** (migration `064`): o anon perdeu o SELECT em 18 tabelas; sobraram as 3 que o hub público consome. Conferido por chamada real: `roteiros` negado, hub em produção 200 com 120 cards.
- **As fontes da checagem estavam públicas.** `ideias.metadata` traz `checagem` (as fontes), `ancora`, `gancho_sugerido`, `gatilho_psicologico`, `harness`, `seed`, `experimento` — e o hub fazia `select('metadata')`, que traz o objeto inteiro. **52 de 60 ideias lidas expunham a checagem**, contra a determinação do dono de que as fontes são sempre internas. Criadas duas views mínimas (`062`, aplicada) com os 7 campos que o hub usa; o hub corrigido está commitado e verificado, **aguardando deploy**.
