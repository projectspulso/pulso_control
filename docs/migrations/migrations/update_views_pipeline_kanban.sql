...
ORDER BY p.data_publicacao_planejada,
    c.nome,
    s.nome,
    p.prioridade DESC;
COMMENT ON VIEW pulso_content.vw_pipeline_calendario_publicacao IS 'View de pipeline para calendário - somente itens com data de publicação planejada';
...
ORDER BY p.status,
    ...
    s.nome,
    p.prioridade DESC;
COMMENT ON VIEW pulso_content.vw_pipeline_kanban IS 'View de pipeline para Kanban - todos os itens organizados por status';
...
SELECT *
FROM pulso_content.vw_pipeline_calendario_publicacao;
GRANT SELECT ON public.vw_pipeline_calendario_publicacao TO anon,
    authenticated;
COMMENT ON VIEW public.vw_pipeline_calendario_publicacao IS 'View pública de pipeline para calendário - acesso via PostgREST';
...
SELECT *
FROM pulso_content.vw_pipeline_kanban;
GRANT SELECT ON public.vw_pipeline_kanban TO anon,
    authenticated;
COMMENT ON VIEW public.vw_pipeline_kanban IS 'View pública de pipeline para Kanban - acesso via PostgREST';
