-- =====================================================
-- LIMPAR DUPLICATAS DE ROTEIROS E IDEIAS
-- =====================================================
-- 1. DELETAR ROTEIROS DUPLICADOS (mantendo o mais antigo)
...
-- 2. DELETAR IDEIAS DUPLICADAS (mantendo a mais antiga ou aprovada)
...
| tabela | total | | -------- | ----- |
| Roteiros | 30 | | Ideias | 128 | | Pipeline | 30 | -- 4. VERIFICAR SE AINDA HÁ DUPLICATAS DE ROTEIROS
SELECT titulo,
    COUNT(*) as quantidade
FROM pulso_content.roteiros
GROUP BY titulo
...
| titulo | quantidade | | --------------------------------- | ---------- |
| A Casa Winchester | 2 | | A Ilha dos Mortos - Poveglia | 2 | | A Garota do Elevador Elisa Lam | 2 | | Priões: O Vírus Indestrutível | 2 | | Bactérias que Controlam Seu Humor | 2 | | Gêmeos Parasitas | 2 | -- 5. VERIFICAR SE AINDA HÁ DUPLICATAS DE IDEIAS
SELECT titulo,
    COUNT(*) as quantidade
FROM pulso_content.ideias
GROUP BY titulo
