...
SET data_prevista = data_prevista + INTERVAL '9 days',
    updated_at = NOW()
WHERE data_prevista IS NOT NULL;
-- Verificar resultado
DO $$
DECLARE count_atualizado INTEGER;
primeira_data DATE;
ultima_data DATE;
BEGIN
SELECT COUNT(*),
    MIN(data_prevista::date),
    MAX(data_prevista::date) INTO count_atualizado,
    primeira_data,
    ultima_data
FROM pulso_content.pipeline_producao
WHERE data_prevista IS NOT NULL;
RAISE NOTICE '✅ PIPELINE atualizado:';
RAISE NOTICE '   - Items com data_prevista: %',
count_atualizado;
RAISE NOTICE '   - Primeira data: %',
primeira_data;
RAISE NOTICE '   - Última data: %',
ultima_data;
END $$;
-- =====================================================
...
SET data_publicacao = (
        '2025-12-10 09:00:00'::timestamp + (ia.ordem / 3) * INTERVAL '1 day' + CASE
    ...
    data_prevista = CASE
        WHEN pp.data_prevista IS NULL THEN '2025-12-10 09:00:00'::timestamp + (ia.ordem / 3) * INTERVAL '1 day' + CASE
...
DO $$
DECLARE count_com_data INTEGER;
primeira_pub DATE;
ultima_pub DATE;
...
SELECT COUNT(*),
    ...
    MAX(data_publicacao::date) - MIN(data_publicacao::date) INTO count_com_data,
    primeira_pub,
    ultima_pub,
    dias_cobertos
FROM pulso_content.pipeline_producao
...
RAISE NOTICE '   - Items agendados: %',
count_com_data;
RAISE NOTICE '   - Primeira publicação: %',
primeira_pub;
RAISE NOTICE '   - Última publicação: %',
ultima_pub;
RAISE NOTICE '   - Período (dias): %',
dias_cobertos;
...
SELECT COUNT(*) INTO total_audios
FROM pulso_content.audios;
SELECT COUNT(*) INTO items_agendados
FROM pulso_content.pipeline_producao
WHERE data_publicacao IS NOT NULL;
RAISE NOTICE '';
RAISE NOTICE '================================================';
...
RAISE NOTICE '  - Ideias aprovadas: %',
total_ideias;
RAISE NOTICE '  - Roteiros criados: %',
total_roteiros;
RAISE NOTICE '  - Áudios gerados: %',
total_audios;
RAISE NOTICE '  - Items agendados: %',
items_agendados;
RAISE NOTICE '';
...
SELECT data_publicacao::date as dia_publicacao,
